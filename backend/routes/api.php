...
use App\Http\Controllers\Apis\V1\Users\Auth\PasswordResetController;
...
Route::prefix('v1/')->group(function () {
    Route::prefix('/users')->name('users.')->group(function () {
        ...
        Route::post('/reset/send', [PasswordResetController::class, 'sendResetLinkEmail'])->name('password.reset.send');
        Route::patch('/reset', [PasswordResetController::class, 'reset'])->name('password.reset');

        Route::middleware(['auth:sanctum', 'can:general'])->group(function () {
            Route::get('/me', [AuthController::class, 'me'])->name('me');
            Route::as('users.')->group(function () {
                Route::patch('reset/{id}', [PasswordResetController::class, 'reset'])->name('password.reset');
            });
